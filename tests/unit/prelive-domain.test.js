...
import { HttpSonosAdapter, validateSonosSettings } from '../../server/services/sonos/HttpSonosAdapter.js'
...
test('Sonos native settings are strictly validated before test-only dry runs', () => { assert.doesNotThrow(() => validateSonosSettings({ loudness: true, surroundMusicMode: 'full', dialogSyncDelay: 3 })); assert.throws(() => validateSonosSettings({ dialogSyncDelay: 6 }), /between/); assert.throws(() => validateSonosSettings({ surroundMusicMode: 'wide' }), /ambient or full/); assert.throws(() => validateSonosSettings({ unknown: true }), /Unsupported/) })
